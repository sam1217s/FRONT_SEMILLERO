          ...
          >
            ...
            <template v-else-if="col.name === 'opciones'">
              <ActionButtons
                ...
                view-tooltip="Ver"
                edit-tooltip="Editar"
                :activate-tooltip="(props.row.estado === 'Inactivo' || props.row.status === 'Inactive') ? 'Activar' : 'Desactivar'"
                :deactivate-tooltip="(props.row.estado === 'Inactivo' || props.row.status === 'Inactive') ? 'Activar' : 'Desactivar'"
            ...
            <!-- Proceso: badge por estado del proceso -->
            <template v-else-if="col.name === 'proceso'">
              <q-badge
                :color="processColor(props.row.status)"
            ...
            <template v-else-if="col.name === 'status' || col.name === 'estado'">
              <q-badge
                :color="(props.row.estado === 'Activo') || (props.row.status === 'Active') || (props.row.status && props.row.status !== 'cancelled' && props.row.status !== 'Inactive') ? 'positive' : 'grey'"
                :label="col.format ? col.format(props.row[col.field]) : props.row[col.field]"
                class="estado-badge"/>
            </template>
          </q-td>
...
// Utils de color para el proceso
function processColor(s) {
  const map = {
    pending: 'warning',
    in_progress: 'primary',
    completed: 'positive',
